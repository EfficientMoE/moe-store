...
from moe_store.wrappers.olmoe_paged_attention import OlmoePagedAttention
...
def test_backend_without_layer_idx_kwarg_is_still_called() -> None:
    """Older backends take no ``layer_idx``; the shim retries without it."""

    class _NoLayerIdxBackend(_CausalSdpaBackend):
        def forward(
            self, query, key, value, attention_metadata=None, scale=None
        ):
            return super().forward(query, key, value, attention_metadata, scale)

    torch.manual_seed(3)
    config = _config()
    shim = OlmoePagedAttention(config, layer_idx=1).eval()
    length = 3
    hidden = torch.randn(1, length, config.hidden_size)
    cos, sin = _rope(config, hidden, torch.arange(length).unsqueeze(0))
    backend = _NoLayerIdxBackend([length])
    OlmoePagedAttention.set_paged_context(backend, _metadata([length]))
    with torch.no_grad():
        out, _ = shim(
            hidden_states=hidden,
...
def test_backend_output_of_wrong_rank_is_rejected() -> None:
    torch.manual_seed(4)
    config = _config()
    shim = OlmoePagedAttention(config, layer_idx=0).eval()
    length = 3
    ...
    OlmoePagedAttention.set_paged_context(_BadBackend(), _metadata([length]))
